...
for idx, i in enumerate(tqdm(uniq_labels, desc='plotting assignments')):
    plt.scatter(data_train[:,0][labels_train==i], data_train[:,1][labels_train==i], color=colors[idx % len(colors)])
...
for idx, i in enumerate(tqdm(uniq_labels)):
    plt.scatter(data[:,0][labels==i], data[:,1][labels==i], color=colors[idx % len(colors)])
